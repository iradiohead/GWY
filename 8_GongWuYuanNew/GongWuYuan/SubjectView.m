...
/*
self.layout_content[CUS_ANSWER] 
self.layout_content[STATUS]
NEED TO REASSIGN VALUE WHEN USER CLICK THE BUTTON, always need to sync with database
    ...
    if (self)
    {
        ...
        if(![pic_name isEqual: @""])
        {
            ...
            for(int i=0; i<pic_name_list.count; i++)
            {
                ...
                [self.scView addSubview:meterialPicLabel];
            }
        }
...
//        [self.scView addSubview:_meterialPicLabel];
        [self.scView addSubview:_pidInTestLabel];
        [self.scView addSubview:_problemStrLabel];
//        [self.scView addSubview:_problemPicLabel];
...
{   //when popup navi viewcontrol, left swipe, then right swipe, create subjectviewcontrol again
    NSString* cusAns = self.layout_content[CUS_ANSWER];
...
- (void)initButton:(UIButton*) button withTitle:(NSString*)title
{
    ...
    if(self.showAnswer)
    {
        if ([self.layout_content[CUS_ANSWER] rangeOfString:title].location != NSNotFound)
        {
            if([self.layout_content[ANSWER] rangeOfString:title].location != NSNotFound)
            {
   ...
   // NSLog(@"bound= %f,frame = %f",self.bounds.size.width,self.frame.size.width);
    ...
    if(self.enumSVFunctionType == enumTrueSubjectType || self.enumSVFunctionType == enumSimulatedSubjectType)
    {
	    ...
	    if(![problem_nsstr isEqual: @""])
	        updateFrame = [self diffrowLayout:problem_nsstr withFrame:CGRectMake(INTERNAL_LEFT_SPACE, updateFrame.height, TEXTWIDTH, 0) withElement:self.problemStrLabel];
	}
	else if(self.enumSVFunctionType == enumSpecializedTrainingType ||self.enumSVFunctionType == enumWrongSetType || self.enumSVFunctionType == enumMyCollectionType )
	{
		NSMutableString *problem_nsstr = self.layout_content[PROBLEM_T];
	    if(![problem_nsstr isEqual: @""])
	        updateFrame = [self diffrowLayout:problem_nsstr withFrame:CGRectMake(LEFT_SPACE, updateFrame.height+PARAGRAPH_SPACE, TEXTWIDTH, 0) withElement:self.problemStrLabel];
	}
