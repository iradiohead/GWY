...
static NSString *startTestButtonStr = @"重新开始";
static NSString *continueTestButtonStr = @"继续答题";

@implementation TestPaperInfoView
//--------------------------------------------
#pragma mark init 
...
- (void)layoutSubviews
{
    ...
    NSString* subCountAndSetTimeStr = [[NSString alloc]
           initWithFormat:@"%@题, 限定时间%@分钟",self.layout_content[0],self.layout_content[2]];
    ...
    updatedFrame = [self sameRowButtonLayoutWithFrame:CGRectMake((self.bounds.size.width-PAPERINFO_BUTTON_WIDTH*2-20)/2 , updatedFrame.height+PARAGRAPH_SPACE, PAPERINFO_BUTTON_WIDTH, PAPERINFO_BUTTON_HEIGHT)
