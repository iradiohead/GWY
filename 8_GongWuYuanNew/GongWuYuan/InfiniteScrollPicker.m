...
- (void)initInfiniteScrollView
{
    ...
    NSLog(@"_itemSize.height = %f, self.frame.size.height =%f",_itemSize.height,  self.frame.size.height);
    ...
    if ( _imageAry.count > 0 )
    {
        ...
        for (int i = 0; i < (_imageAry.count * 5); i++)
        {
            ...
            //kjin add TapGesture when tap the image, call super view controller's action callback
            temp.userInteractionEnabled = YES;
            SEL selector = @selector(touchPhotoAction:);
            if ([[self firstAvailableUIViewController] respondsToSelector:selector])
            {
                UITapGestureRecognizer *singleTap = [[UITapGestureRecognizer alloc] initWithTarget:[self firstAvailableUIViewController]
                                                                                            action:selector] ;
                [temp setTag:i%_imageAry.count];
                [temp addGestureRecognizer:singleTap];
            }
            // use arc [singleTap release];
...
-(void)touchPhotoAction:(UIGestureRecognizer *)gestureRecognizer{
    ...
    NSLog(@"tPhotoIndex __%d",tPhotoIndex );
